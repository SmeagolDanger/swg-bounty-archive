...
import { boardCatalogSchema, bountySchema, leaderboardSchema, winsSchema, officersSchema, isExpectedNullableType } from "./schemas";
...
async function recordSchemaAndUnknownFields(
  ...
  if (inserted.rows[0]?.inserted && prior && prior.signature !== shape.signature) {
    ...
    const widenedTypes = diff.changedTypes.filter((change) => change.to.some((type) =>
      !change.from.includes(type) && !isExpectedNullableType(change.path, type)));
