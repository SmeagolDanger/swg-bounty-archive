...
import { overlayRows, overlayStats, type OverlayDossier, type OverlayResult } from "@/lib/overlay/model";

// OBS browser-source overlay: a game-HUD "recent bounties" panel driven by
// the public dossier API. The page body is transparent; only the panel is
// painted, so it composites cleanly over gameplay.
...
export function BountyOverlay({ name, rows: rowCount, refresh, avatar, title }: {
  name: string; rows: number; refresh: number; avatar?: string; title: string;
}) {
  ...
  useEffect(() => {
    ...
    const load = async () => {
      try {
        ...
        if (!cancelled && !dossier) setError(cause instanceof Error ? cause.message : "Archive unreachable");
      }
    };
    void load();
  ...
  const rows = dossier ? overlayRows(dossier, rowCount, now) : [];
  ...
  return <div className="bounty-overlay" data-overlay-root data-overlay-ready={dossier || error ? "" : undefined}>
    <div className="panel-frame">
      ...
      <div className="board">
        ...
        {error && <div className="board-row board-note">{error}</div>}
      ...
      {stats && <footer className="panel-foot">
        <div><Crosshair/><span>Today:</span><b className="tone-good">{stats.todayClaimed} claimed</b></div>
        <div><Clipboard/><span>Cycle contracts:</span><b>{stats.cycleContracts ?? "—"}</b></div>
        <div><Star/><span>Best payout:</span><b className="tone-gold">{stats.bestPayout ? `${stats.bestPayout} cr` : "—"}</b></div>
      </footer>}
    </div>
  </div>;
}
