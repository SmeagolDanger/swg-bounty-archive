...
export async function Dossier({ id, type }: { id: string; type: "player" | "guild" | "city" }) {
  ...
  const repeatedRivalries = rivalries.filter((row) => Number(row.encounters) >= 2);
  const nemesis = [...rivalries].filter((row) => Number(row.losses) > 0).sort((a, b) => Number(b.losses) - Number(a.losses) || Number(a.win_rate) - Number(b.win_rate))[0];
  const longest = [...repeatedRivalries].sort((a, b) => (new Date(b.last_event_at).getTime() - new Date(b.first_event_at).getTime()) - (new Date(a.last_event_at).getTime() - new Date(a.first_event_at).getTime()))[0];
  ...
  return <div className="shell">
    ...
    {type === "player" && hunter && <>
      ...
      <section className="section"><div className="dashboard-grid">
        <div className="panel"><div className="panel-header"><h3>Encounter performance</h3><span className="chip">Claims vs failures</span></div><HunterActivityChart data={data.dailyActivity}/></div>
        <div className="panel"><div className="panel-header"><h3>When targeted</h3><span className="chip">Separate role</span></div><div className="target-summary"><div><span>Targeted</span><b>{integer(target?.encounters)}</b></div><div><span>Survived</span><b className="health-good">{integer(target?.survived)}</b></div><div><span>Killed</span><b className="health-bad">{integer(target?.killed)}</b></div><div><span>Survival rate</span><b>{percent(target?.survival_rate)}</b></div></div><p className="stat-definition">A failed contract counts as a target survival. These events are deliberately excluded from the hunter-role win rate.</p></div>
      </div></section>
      <section className="section"><div className="section-head"><div><span className="kicker">Both encounter roles</span><h2>Rivalry intelligence</h2></div><Link href="/rivalries">Open rivalry network →</Link></div>
        <dl className="metrics"><div className="metric"><dt>Repeated rivalries</dt><dd>{integer(repeatedRivalries.length)}</dd><small>opponents encountered 2+ times</small></div><div className="metric"><dt>Nemesis</dt><dd className="compact-dd">{nemesis?.opponent ?? "—"}</dd><small>{nemesis ? `${nemesis.losses} head-to-head losses` : "no archived losses"}</small></div><div className="metric"><dt>Longest rivalry</dt><dd className="compact-dd">{longest?.opponent ?? "—"}</dd><small>{longest ? `${longest.encounters} encounters since ${date(longest.first_event_at)}` : "needs repeat encounters"}</small></div><div className="metric"><dt>Revenge kills</dt><dd>{integer(rivalries.reduce((sum, row) => sum + Number(row.revenge_kills), 0))}</dd><small>killed the pair’s previous killer</small></div></dl>
        <div className="panel rivalry-panel"><div className="panel-header"><h3>Head-to-head records</h3><span className="chip">Wins include survivals</span></div>{rivalries.length ? <div className="data-scroll"><table className="data-table"><thead><tr><th>Opponent</th><th>Record</th><th>Claims</th><th>Survivals</th><th>Revenge</th><th>Last encounter</th></tr></thead><tbody>{rivalries.slice(0, 12).map((row) => <tr key={row.opponent_key}><td><Link className="entity-link" href={`/rivalry/${id}/${encodeURIComponent(row.opponent)}`}><b>{row.opponent}</b></Link></td><td><span className="health-good">{row.wins}W</span> <span className="health-bad">{row.losses}L</span><small>{percent(row.win_rate)} win rate</small></td><td>{row.claims}</td><td>{row.survivals}</td><td>{row.revenge_kills}</td><td>{date(row.last_event_at)}</td></tr>)}</tbody></table></div> : <div className="empty">No head-to-head activity is archived for this identity.</div>}</div>
      </section>
    </>}
    ...
    <section className="section"><div className="dashboard-grid"><div className="panel"><div className="panel-header"><h3>Rank history</h3><span className="chip">Source observations</span></div><HistoryChart rows={data.history}/></div><div className="panel"><div className="panel-header"><h3>Contract targets</h3><span className="chip">Hunter role only</span></div>{type === "player" && data.opponents.length ? data.opponents.map((row, index) => <div className="opponent-row" key={row.opponent}><span className="rank">{index + 1}</span><span><b>{row.opponent}</b><small>{integer(row.encounters)} contracts · {percent(row.win_rate)} claim rate</small></span><span className="record"><b className="health-good">{row.wins}W</b> <b className="health-bad">{row.losses}L</b></span></div>) : <div className="empty">{type === "player" ? "No hunter-role matchups are available from the archive." : "Contract targets are available only for hunter profiles."}</div>}</div></div></section>
