...
export function CompareBuilder({ initialIds }: { initialIds: string[] }) {
  ...
  useEffect(() => { if (query.length<2)return; const timer=setTimeout(async()=>{const response=await fetch(`/api/search?q=${encodeURIComponent(query)}`);if(response.ok)setResults(((await response.json()).results as Result[]).filter((item)=>item.participant_type==="player"));},180);return()=>clearTimeout(timer);},[query]);
