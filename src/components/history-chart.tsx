...
export function HistoryChart({ rows }: { rows: Array<Record<string,unknown>> }) {
  const byDate=new Map<string,Record<string,unknown>>(); for(const row of [...rows].reverse()){const date=localDay(row.source_fetched_at as string);const item=byDate.get(date)??{date};item[row.leaderboard_id as string]=Number(row.rank);byDate.set(date,item);}
