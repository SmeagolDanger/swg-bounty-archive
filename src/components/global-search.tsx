...
export function GlobalSearch() {
  ...
  useEffect(() => {
    ...
    const timer = setTimeout(async () => {
      try {
        ...
        if (response.ok) {
          const list = (await response.json()).results as Result[];
          setResults(list);
  ...
  const dismiss = () => { setResults([]); setNoMatches(false); };
  return <div className="global-search"
    ...
    <input value={query} onChange={(event) => { const value=event.target.value; setQuery(value); if(value.trim().length<2)dismiss(); }} placeholder="Search dossiers…" aria-label="Search hunters, guilds, and cities" />
    ...
    {noMatches && results.length === 0 && query.trim().length >= 2 && <div className="search-results"><span className="search-empty">No dossiers match &ldquo;{query.trim()}&rdquo;.</span></div>}
  </div>;
}
