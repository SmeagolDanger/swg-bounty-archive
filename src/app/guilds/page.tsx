...
export default async function GuildsPage({ searchParams }: { searchParams: Promise<Record<string, string | string[] | undefined>> }) {
  ...
  return <div className="shell">
    ...
    <div className="panel"><div className="panel-header"><h3>{number(data.rows.length)} guild intelligence files</h3><Link className="chip" href="/leaderboards?subject=guild">Source standings →</Link></div><div className="data-scroll"><table className="data-table guild-table mobile-cards"><thead><tr><th>Guild</th><th>Source standing</th><th>Roster</th><th>Active</th><th>Record</th><th>Win rate</th><th className="numeric">Credits</th></tr></thead><tbody>{data.rows.map((row) => <tr key={row.id}><td data-label="Guild" className="card-title"><Link className="entity-link" href={`/guild/${row.id}`}><b>{row.current_name}</b></Link><small>{row.guild_abbreviation || "No abbreviation"}</small></td><td data-label="Source standing">{row.rank ? <>#{row.rank}<small>{number(row.score)} total-kills score</small></> : "—"}</td><td data-label="Roster">{row.roster_size}</td><td data-label="Active">{row.active_hunters}</td><td data-label="Record"><span className="health-good">{row.wins}W</span> <span className="health-bad">{row.losses}L</span></td><td data-label="Win rate">{percent(row.win_rate)}</td><td data-label="Credits" className="numeric credits">{number(row.credits)} cr</td></tr>)}</tbody></table></div>{!data.rows.length && <div className="empty">No guilds match this filter.</div>}</div>
  </div>;
}
