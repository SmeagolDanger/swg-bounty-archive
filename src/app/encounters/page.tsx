...
export default async function EncountersPage({ searchParams }: { searchParams: Promise<Record<string, string | string[] | undefined>> }) {
  ...
  return <div className="shell">
    ...
    <div className="panel"><div className="panel-header"><h3>{data.total.toLocaleString("en-US")} records</h3><span className="chip">Newest first</span></div><div className="data-scroll"><table className="data-table"><thead><tr><th>Timestamp</th><th>Hunter</th><th>Outcome</th><th>Target</th><th className="numeric">Payout</th></tr></thead><tbody>{data.rows.map((row) => <tr key={row.id}><td><LocalDateTime value={row.event_at}/></td><td>{row.hunter_participant_id ? <Link className="entity-link" href={`/hunter/${row.hunter_participant_id}`}>{row.hunter_name}</Link> : row.hunter_name}</td><td><span className={`status ${row.outcome.toLowerCase()}`}>{row.outcome === "KILL" ? "Collected" : "Failed"}</span></td><td>{row.target_participant_id ? <Link className="entity-link" href={`/hunter/${row.target_participant_id}`}>{row.target_name}</Link> : row.target_name}</td><td className="numeric credits">{row.outcome === "KILL" ? `${Number(row.credits).toLocaleString("en-US")} cr` : "—"}</td></tr>)}</tbody></table></div>
