...
export default async function RivalriesPage({ searchParams }: { searchParams: Promise<Record<string, string | string[] | undefined>> }) {
  ...
  return <div className="shell">
    ...
    <dl className="metrics">
      ...
      <div className="metric"><dt>Longest span</dt><dd>{String(data.summary.longest_span ?? "0 days").replace(" days", "d")}</dd><small>first event to latest event</small></div>
    </dl>
    ...
    <div className="panel"><div className="panel-header"><h3>{number(data.total)} rivalry files</h3><span className="chip">Both encounter roles</span></div><div className="data-scroll"><table className="data-table rivalry-table"><thead><tr><th>Matchup</th><th>Head-to-head</th><th>Encounters</th><th>Revenge</th><th>Span</th><th className="numeric">Claims</th></tr></thead><tbody>{data.rows.map((row) => {
      ...
      return <tr key={`${row.side_a_key}-${row.side_b_key}`}><td>{detail ? <Link className="entity-link matchup" href={detail}>{content}</Link> : <span className="matchup">{content}</span>}</td><td><span className="health-good">{row.side_a_wins}</span>–<span className="health-bad">{row.side_b_wins}</span></td><td>{row.encounters}</td><td>{row.revenge_kills}</td><td>{span(row.first_event_at, row.last_event_at)}</td><td className="numeric credits">{row.claims} · {number(row.credits)} cr</td></tr>;
