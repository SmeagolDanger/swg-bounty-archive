...
export default async function WeeklyReportPage({ searchParams }: { searchParams: Promise<Record<string, string | string[] | undefined>> }) {
  ...
  const utcDate = (value: Date | string) => new Intl.DateTimeFormat("en-US", { month: "short", day: "numeric", year: "numeric", timeZone: "UTC" }).format(new Date(value));
  const utcStamp = (value: Date | string) => new Intl.DateTimeFormat("en-US", { month: "short", day: "numeric", year: "numeric", hour: "2-digit", minute: "2-digit", timeZone: "UTC", timeZoneName: "short" }).format(new Date(value));
  const shareVM = report.cycle && report.summary ? {
  ...
  return <div className="shell weekly-report">
    <header className="page-head report-page-head">
      ...
      <div className="report-title-row"><div><h1>Weekly cycle report</h1><p>A bounded report using the exact weekly dates supplied by the SWG Legends leaderboard source.</p></div>{shareVM && <ReportExportButton vm={shareVM}/>}</div>
    </header>
