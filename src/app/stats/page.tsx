...
export default async function StatsPage() {
  ...
  return <div className="shell">
    ...
    <section className="section"><div className="dashboard-grid">
      ...
      <div className="panel"><div className="panel-header"><h3>Archive window</h3><span className="chip">Event time</span></div><div className="target-summary"><div><span>First event</span><b className="compact-value">{data.summary.history_start ? <LocalDateTime value={data.summary.history_start}/> : "Pending"}</b></div><div><span>Latest event</span><b className="compact-value">{data.summary.history_end ? <LocalDateTime value={data.summary.history_end}/> : "Pending"}</b></div><div><span>Claims</span><b className="health-good">{number(kills?.encounters)}</b></div><div><span>Failures</span><b className="health-bad">{number(failures?.encounters)}</b></div><div><span>Unique event names</span><b>{number(data.summary.unique_names)}</b></div><div><span>Groups tracked</span><b>{number(data.summary.guilds)} guilds · {number(data.summary.cities)} cities</b></div><div><span>Encounters today</span><b>{number(data.summary.encounters_today)}</b></div><div><span>Discord hunt feeds</span><b>{number(data.summary.discord_feeds)} {Number(data.summary.discord_feeds) === 1 ? "server" : "servers"}</b></div></div></div>
    </div></section>
    <section className="section"><div className="dashboard-grid">
