...
export async function GET(request: Request) {
  ...
  const limit = z.coerce.number().int().min(1).max(200).catch(60).parse(url.searchParams.get("limit") ?? undefined);
  const character = url.searchParams.get("character")?.trim() || null;
  const purchases = await pool.query(
    `SELECT id, character_name, item_name, seller, credits::float8 AS credits, purchase_type, occurred_at
     FROM mail_purchases WHERE user_id=$1 AND ($2::text IS NULL OR character_name=$2)
     ORDER BY occurred_at DESC LIMIT $3`,
    [user.id, character, limit],
  );
  return Response.json({ purchases: purchases.rows });
}
