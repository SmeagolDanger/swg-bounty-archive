...
// Bounded by the 14-day combat retention window.
export async function GET(request: Request) {
  ...
  const days = z.coerce.number().int().min(1).max(14).catch(3).parse(url.searchParams.get("days") ?? undefined);
  const events = await pool.query(
    `SELECT kind, source, target, ability, amount::float8 AS amount, flag, occurred_at
     ...
     ORDER BY occurred_at ASC, id ASC LIMIT 100000`,
    [user.id, String(days)],
  );
